...
def create_box(a,n):
	...
	# Display the image
    ax.imshow(neworiginal)
    w = (int)(neworiginal.shape[1])
    h = (int)(neworiginal.shape[0])
	# Create a Rectangle patch
    if n==0:
        rect = patches.Rectangle((5,5 ), w-10, h-10, linewidth=5, edgecolor='r', facecolor='none')

        # Add the patch to the Axes
        ax.add_patch(rect)
        ax.text(20, 25, 'black rot: %f%%'%a, style='italic',
                bbox={'facecolor': 'yellow', 'alpha': 22, 'pad': 10})
    else:
        rect = patches.Rectangle((5, 5), w - 10, h - 10, linewidth=5, edgecolor='blue', facecolor='none')

        # Add the patch to the Axes
        ax.add_patch(rect)
        ax.text(20, 25, 'Healthy!', style='italic',
                bbox={'facecolor': 'yellow', 'alpha': 22, 'pad': 10})
...
cv2.imwrite('means shift image.png', img)
...
cv2.imwrite('Contour.png', neworiginal)
...
cv2.imwrite('rectangle Roi.png', roi)
img = roi
...
cv2.imwrite('masked our img.png', mask)
...
for x in range(len(contours)):
    ...
    cv2.imwrite('Contour masked.png', originalroi)
